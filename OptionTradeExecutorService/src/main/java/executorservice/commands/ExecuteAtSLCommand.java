...
public class ExecuteAtSLCommand implements ICommand<ClientOrder> {

    private ClientOrder clientOrder;
    private KiteConnect kiteConnect;
    private OrderCache orderDataCache;
    private String clientUserId;

    public ExecuteAtSLCommand(ClientOrder clientOrder, KiteConnect kiteConnect,
                              OrderCache orderDataICache, String clientUserId) {
        this.kiteConnect = kiteConnect;
        this.clientOrder = clientOrder;
        this.orderDataCache = orderDataICache;
    ...
    public ITradeExecutionResult execute() {
        ...
        try {
            ...
            //orderParams.stoploss = Util.getDefaultStopLossPrice(clientOrder.getTriggerPrice(), clientOrder.getStopLossPrice(), clientOrder.getLots(), clientOrder.getLotSize());
            ...
            System.out.println("Placing SL order with client order :" + clientOrder.toString() + "and order parameter: " + getOrderParamString(orderParams));
            OrderResponse order = kiteConnect.placeOrder(orderParams, Constants.VARIETY_REGULAR);
            orderDataCache.add(clientOrder.getMasterOrderId(), new OrderData(order.orderId, kiteConnect.getUserId()));
            System.out.println("SL Order successfully place with order id: " + order.orderId + "all order parameter: " + getExecutedOrderString(order));
    ...
    public boolean canExecute() {
        if (!StringUtils.isEmpty(clientOrder.getTradingSymbol()) &&
                ...
                Double.compare(clientOrder.getTriggerPrice(), 0) != 0) {
            return true;
        }
        return false;
    }
    ...
    private String getOrderParamString(OrderParams orderParams) {
        ...
        orderPramBuilder.append("OrderParams [quantity :" + orderParams.quantity)
                ...
                .append(", triggerprice :" + orderParams.triggerPrice)
                .append("]");
...
//    }
    ...
    public String getExecutedOrderString(OrderResponse orderResponse)
    {
        StringBuilder executedOrderBuilder = new StringBuilder();
