...
public class ModifySLOrderToExecuteOnMarketCommand implements ICommand<ClientOrder>{

    private final ClientOrder clientOrder;
    private final KiteConnect kiteConnect;
    private final String userId;

    public ModifySLOrderToExecuteOnMarketCommand(ClientOrder clientOrder, KiteConnect kiteConnect, String userId)
    {
    ...
    public ITradeExecutionResult execute() {
        try {
            ...
            System.out.println("Modifying SL order to execute on market :" + clientOrder.getOrderId()+ "and order parameter: "+ getOrderParamString(orderParams));
            Order order = kiteConnect.modifyOrder(clientOrder.getOrderId(), orderParams, Constants.VARIETY_REGULAR);
            System.out.println("SL order modified to be executed on market order id: " + order.orderId + "all order parameter: " + getExecutedOrderString(order));
